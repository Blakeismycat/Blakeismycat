...
createwarning("SturdyWare | Boosters", "BlueTexturePack - Un1versxl!   ", 5) -- totally real
    ...
    local hpbar = {Enabled = false}
			...
			hpbar = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
				...
				NoRoot = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
					...
					["Function"] = function(callback)
						if callback then
							...
							task.wait()
							primary.Parent = char
							warningNotification("SturdyWare | Boosters", "HumanoidRootPart Destroyed! Reset your character to disable.", 3)
						end
					end
				})
				...
				OldTexturepack = GuiLibrary["ObjectsThatCanBeSaved"]["WorldWindow"]["Api"].CreateOptionsButton({
					["Name"] = "SturdyWare | Boosters txt pack",
					   ["Function"] = function(Callback)
							Enabled = Callback
							if Enabled then
							   local cam = workspace.CurrentCamera
...
PistonwareAmbience = GuiLibrary["ObjectsThatCanBeSaved"]["RenderWindow"]["Api"].CreateOptionsButton({
	["Name"] = "SturdyWare | Boosters Ambience",
	["Function"] = function(callback)
		if callback then
			local Lighting = game:GetService("Lighting")
...
s.Parent = Lighting
		...
		createwarning("SturdyWare | Boosters", "Join A New Match To Reset Skybox And Ambience.", 3)
		end
	end
})
...
game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(x,y-10,z)
		...
		SizeChanger = GuiLibrary["ObjectsThatCanBeSaved"]["RenderWindow"]["Api"].CreateOptionsButton({
			...
			["Function"] = function(callback)
				if callback then
					...
					createwarning("SturdyWare | Boosters", "Disabled Next Time You Die", 3)
				end
			end
		})
	end)
								...
								runcode(function()
									...
									runcode(function()
										...
										speedgobrrrrrrr = GuiLibrary["ObjectsThatCanBeSaved"]["BlatantWindow"]["Api"].CreateOptionsButton({
											["Name"] = "SturdyWare Speed V.1",
											["HoverText"] = "SturdyWare | Boosters only :)",
											["Function"] = function(callback)
												if callback then
	...
	runcode(function()
		local AntiVoid2 = {Enabled = false}
			...
			Name = "SturdyWare | Boosters AntiVoid(V2) ",
			Function = function(callback) 
				if callback then
        ...
        local function createchattag(name, tagname)           
            local Players = game:GetService("Players")
                    ...
                    ChatTag[yes] =
                        {
                            TagText = tagname or "SturdyWare| Boosters USER",
            ...
            runcode(function()
                ...
                local OwnerID = 417320161
                local Detected = game.Players.LocalPlayer
            
                Detector = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
                    Name = "SturdyWare ClientOwnerFinder",
                    HoverText = "Detects the owner of SturdyWare",
                    Function = function(callback)
                        if callback then
                            if game.Players:GetPlayerByUserId(OwnerID) then
                                createwarning("SturdyWare | Boosters", "The SturdyWare Owner has been detected", 10)
                                createchattag("SturdyWare | Boosters", "SturdyWare OWNER")
                            end
                        end
                    end
                })
			...
			["Function"] = function(callback)
				if callback then
					...
					if game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
						...
						createwarning("SturdyWare | Boosters", "Loaded Infinite Yield", 2)
					else
					end
				end
			end
